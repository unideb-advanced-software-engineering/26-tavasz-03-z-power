...
model {
  ...
  zpower = system 'Z-Power' {
    ...
    // - A persistenceWorker csak annyit csinál, hogy a normalizált adatot beírja a time-series db-be. Egyetlen instance-ban fut: batch-elt írással bőven elbírja a forgalmat (a TimescaleDB szereti a nagy batcheket), ráadásul így biztosított a beérkezési sorrend megtartása az idősoros tárolásnál. Ha leállna, a normalizedTopic puffereli az adatot — nem vész el, csak késik a perzisztálás.
    ...
    eda_subsystem = subsystem 'Event-Driven Subsystem'{
        ...
        schemaRegistry = multipleInstanceService 'Schema Registry' {
          description '50+ termelői séma regisztere, verziózva (REQ-02). Konfigurálhatóan bővíthető. HA klaszterben fut, mert minden normalizálás ettől függ.'
        }
        ...
        notificationService = multipleInstanceService 'Notification Service' {
          description 'E-mail és webhook riasztások küldése üzemzavarok esetén (REQ-10). Több instance burst-terhelés (pl. vihar utáni tömeges üzemzavarok) és HA miatt.'
        }

        persistenceWorker = service 'Persistence Worker' {
          description 'Normalizált adatok tartós tárolásra írása az idősor-adatbázisba batch-elt módon. Egyetlen instance: a DB a szűk keresztmetszet, nem a worker; emellett így biztosított az idősor beérkezési sorrendje (FIFO a topicról). Kiesés esetén a normalizedTopic puffereli az adatot, így nincs adatvesztés.'
        }
    ...
    microkernel_subsystem = subsystem  'Microkernel Subsystem' {
        ...
        microkernelCore = multipleInstanceService 'Microkernel Core' {
          description 'Központi mag pluginekkel: autentikáció, RBAC, routing. Pluginek: valós idejű dashboard (REQ-12..14), historikus analitika (REQ-17..20), export (REQ-21..24), előrejelzés (REQ-25..29). Stateless (állapot a mainDb-ben), több klienst szolgál ki egyszerre — HA és load miatt több példányban fut load balancer mögött.'
        }

        dispatcherApi = multipleInstanceService 'Dispatcher Integration API' {
          description 'Dedikált API-réteg a diszpécserközpontnak. Verziózott, dokumentált sémák. Kritikus külső integráció — az ASR szerint nem opció a kiesése (energiahálózat üzemeltetését veszélyezteti), ezért HA klaszterben fut.'
        }
